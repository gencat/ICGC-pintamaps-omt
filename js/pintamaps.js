...
function processaEstil(estil, nouMapa) {
	...
	$.getJSON(estil)
	.done(function (nou_estil, textStatus) {

		if (nouMapa) {

			//style.layers = nou_estil.layers;
			mapStyle = nou_estil;
			//colorMatrix = nou_estil.colorMatrix;
...
function actualitzaMapa(mapa, aplicaZoom) {
	...
	//if(aplicaZoom){
	map.setCenter([parseFloat(mapa.center.lng).toFixed(4), parseFloat(mapa.center.lat).toFixed(4)]);
...
function randomizeColor() {
	...
	$('#c_edi').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
	$('#c_illes').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
	$('#c_polurbans').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
	$('#c_car').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
	$('#c_bos').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
	$('#c_rocam').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
	$('#c_platges').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
	$('#c_fons').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
	$('#c_rius').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
	$('#c_aigues').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
	$('#c_ferrocarrils').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
	$('#c_carrers').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
	$('#c_camins').colorpicker('setValue', chroma.random().alpha(Math.random().toFixed(2)).css());
...
jQuery('#t_fons div').on('click', function (e) {
	...
	jQuery('#bt_export').on('click', function () {
		...
		//var msg = '{"layers":' + JSON.stringify(obj) + ',"colorMatrix":' + JSON.stringify(colorMatrix) + ',"mapa":{"center":[' + map.getCenter().toArray() + '],"zoom":' + map.getZoom() + ',"pitch":' + map.getPitch() + ',"bearing":' + map.getBearing() + '}}';
	...
	for (var i = 0; i < styleGL.layers.length; i++) {
		...
		for (var j = 0; j < nomsCapes.length; j++) {
			if (tema.indexOf(nomsCapes[j]) != -1) {
				...
				if (event) {
					colorRGBA = 'rgba(' + event.color.toRGB().r + ',' + event.color.toRGB().g + ',' + event.color.toRGB().b + ',' + event.color.toRGB().a + ')'
				} else {
...
function preparaColorCapes() {
	...
	$('#c_edi').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {
	...
	$('#c_illes').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {
	...
	$('#c_polurbans').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {
	...
	$('#c_car').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {
	...
	$('#c_bos').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {
	...
	$('#c_rocam').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {
	...
	$('#c_platges').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {
	...
	$('#c_fons').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {			
			if(!pattern_map){
	...
	$('#c_rius').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {
	...
	$('#c_aigues').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {
	...
	$('#c_toponimia').colorpicker(options).on('changeColor.colorpicker', function(event){
	if(socInici){changeColorLayer(event, "toponimia_cap#toponimia_nucli",  'text-color',10);
	});
	 */
	//Ferrocarrils
	$.extend(options, {
	...
	$('#c_ferrocarrils').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {
	...
	$('#c_carrers').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {
	...
	$('#c_camins').colorpicker(options).on('changeColor.colorpicker', function (event) {
		if (socInici) {
...
//form.widthInput.addEventListener('change', function(e) {
    ...
    if (val > 0) {
        if (val * dpi > maxSize) {
            ...
            errors.width.msg = 'El valor màxim de l´amplada és ' + maxSize +
                'px, però l´amplada entrada és ' + (val * dpi) + 'px.';
...
//form.heightInput.addEventListener('change', function(e) {
    ...
    if (val > 0) {
        if (val * dpi > maxSize) {
            ...
            errors.height.msg = 'El valor màxim de l´alçada és ' + maxSize +
                'px, però l´alçada entrada és ' + (val * dpi) + 'px.';
        } else if (val * window.devicePixelRatio * 96 > maxSize) {
            errors.height.state = true;
...
*/
/*
if (jQuery('#unitOptions').val() == 'mm') {
    jQuery('#widthInput').val() *= 25.4;
    jQuery('#heightInput').val() *= 25.4;
}
*/
